 ...
 * Formally retired the `modsTemps.pl` script now that we have all MODS environmental sensor data reported to the observatory Data Dictionary (DD). Third-party applications requiring access to environmental sensor data must query the DD, but if on a mods server, can use `vueinfo env`
