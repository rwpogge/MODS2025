/** \file utilities.c
 * \brief client for sending commands to the utilities via the 
 * MODS WAGO Controller Service (mmcService). Other commands
 * are WAGO controller commands. Can be found at address:
 * http://www.astronomy.ohio-state.edu/~rgonzale/README_WAGO
...
int utilities(char *host, char *port, char str1[], char str2[])
{
  ...
  sprintf(send_buff, "util %s", str1); // Form message given by caller
  len=strlen(send_buff)+1;

  send_buff[len]='\0'; // prevent segmentation faults
  ...
  strcpy(str2,recv_buff); // send reply message back to caller
