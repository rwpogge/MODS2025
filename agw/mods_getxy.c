 ...
 * mirror of the specified MODS AGW Unit via the agwServer.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int 
mods_getxy(char *host, int agw_no, struct spos_xy *posxy, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (len < 0) {
    ...
    if(strstr(recv_buff,"=")) {   // parse received message
      for(i=0;recv_buff[i]!='=';i++);
      i++;
      posxy->posx=atof(&recv_buff[i]); // X axis Guide Probe position.
      
      for(;recv_buff[i]!='=';i++);
      i++;
      posxy->posy=atof(&recv_buff[i]); // Y axis Guide Probe position.
      ierr=0;
    }
  }
