...
int
initTrans(char *host, int agw_no, struct sac_xy *sacxy)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (len < 0) {
    ...
    GetArgs(recv_buff,20,holder);
    sacxy->xref_ccd[agw_no]=atof(holder);
    GetArgs(recv_buff,21,holder);
    sacxy->yref_ccd[agw_no]=atof(holder);
    GetArgs(recv_buff,22,holder);
    sacxy->dx_wfs[agw_no]=atof(holder);
    GetArgs(recv_buff,23,holder);
    sacxy->dy_wfs[agw_no]=atof(holder);
