...
int
CCDtoAGW(char *host, int agw_no, double xgp, double ygp, struct tc_xy *pxy, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  sprintf(send_buff,"ccdtoagw %d %f %f",agw_no, xgp,ygp);
  ...
  if (len < 0) {
    ...
    pxy->dgp_x=atof(holder);
    GetArgs(recv_buff,5,holder);
    pxy->dgp_y=atof(holder);
