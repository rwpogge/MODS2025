/** \file init.c
 * \brief All purpose client for resetting the MODS AGW.
 */
...
int reset(char *host, int agw_no, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  strcpy(send_buff,"reset"); // form connection with agwService
  ...
  len = recv(conn, recv_buff, sizeof(recv_buff), 0); // read msg to agwService
