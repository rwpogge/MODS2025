/** \file mods_setfilter.c
 * \brief Client stub for driving the Filter to a new position
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int 
mods_setfilter(char *host, int agw_no, int new_pos, double velocity, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  sprintf(send_buff,"setfilter %d %f %d", new_pos, velocity, options);
