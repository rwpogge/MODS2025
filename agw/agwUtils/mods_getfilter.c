...
int 
mods_getfilter(char *host, int agw_no, int *filter, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  conn = islMakeContact(comp, app); // form connection to agwService
  if (conn < 0) {
    ierr=3;
  ...
  if (len < 0) {
    ...
    for(i=0;recv_buff[i]!='=';i++); // parse the answer string
    i++;   
    *filter=atoi(&recv_buff[i]);
    ierr=0;
  }
