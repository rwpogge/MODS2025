/** \file driftvec.c
 * \brief This client sends parameters to agwService. agwService sends
 *        confermation.
 */
...
int driftvec(char *host, int agw_no, double xval, double yval, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (MODS_1_PORT != 0)
    if(!agw_no)
      app = (islnum) MODS_1_PORT; // convert argument to binary formatislnum
    else
      app = (islnum) MODS_2_PORT; // convert argument to binary formatislnum
  else
  ...
  sprintf(send_buff,"driftvec %f %f",xval,yval);
