 ...
 */
#ifndef MODSCONTROL_H
#define MODSCONTROL_H

#include "agwcomm.h"

typedef struct sac_xy
 {
   double sfp_x0[2];
   double sfp_xx[2];
   double sfp_xy[2];
...
//  int mods_getparam(char *host, int agw_no, char [], int port, char [], char []);
  int mods_getparam(char *host, int agw_no, char [], int motor, char [], int,int);
 ...
 * @param host: the host the TCP Server is residing on
 * @param agw_no: adresses the AGW
 * @param xgp_x: x coordinate of the new position
 * @param xgp_y: y coordinate of the new position
 * @param xsfp: get converted X SFP
 * @param ysfp: get converted Y SFP
 * (INTERNALLY CASTED TO INTEGER, YET!!!)
 * @param options: \n
 * OPT_INC: move incremental
 * @return an error value
 */
  int initTrans(char *host, int agw_no, sac_xy *xy);
  int AGWtoSFP(char *host, int agw_no, double x, double y, tc_xy *pxy, int options);
  int SFPtoAGW(char *host, int agw_no, double x, double y, tc_xy *pxy, int options);
  int CCDtoSFP(char *host, int agw_no, double x, double y, double dxref, double dyref, tc_xy *pxy, int options);
  int CCDtoAGW(char *host, int agw_no, double x, double y, tc_xy *pxy, int options);
  int mods_initTrans(char *host, int agw_no);//, sac_xy *xy);
  int mods_SFPToAGW(int agw_no, double x, double y, double *pgpx, double *pgpy, int options);
  int mods_AGWToSFP(int agw_no, double x, double y, double *psfpx, double *psfpy, int options);
  int mods_CCDToSFP(int agw_no, double x, double y, double dxref, double dyref, double *pdsfpx, double *pdsfpy,int options);
  int mods_CCDToAGW(int agw_no, double dxccd,double dyccd,double *dxagw, double *dyagw, int options);
