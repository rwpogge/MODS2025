/** \file mods_getpowerstatus.c
 * \brief Client for obtaining the WFS and AGC power status
 * via the agwServer.
 */
#include <stdio.h>
#include <stdlib.h>
...
int 
mods_getpowerstatus(char *host, int agw_no, int *status)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (len < 0) {
    ...
    if(strstr(recv_buff,"OK") && strstr(recv_buff,"ON")) status=1;
    else status=0;
  }  

  (void) fflush(stdout);

  sprintf(send_buff,"util wfs");
  len=strlen(send_buff)+1;
  ...
  if (len < 0) {
    ...
    if(strstr(recv_buff,"OK") && strstr(recv_buff,"ON")) status++;
    else status=0;
  }  
