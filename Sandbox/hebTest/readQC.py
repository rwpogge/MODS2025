#!/usr/bin/env python
'''
Read out QCs, do basic stats.  Lightweight
way to watch QCs during testing and tweaking
when the stripchart plot is not going to work
(e.g., long distance support)

2025 Aug 26 - rwp/osu (last mod)
'''
...
m1rHost = '192.168.139.141' # MODS1 Red
m1bHost = '192.168.139.142' # MODS1 Blue
m2rHost = '192.168.139.241' # MODS2 Red
m2bHost = '192.168.139.242' # MODS2 Blue
m1dHost = '192.168.139.41'  # MODS1-dev Tucson Lab

wagoHost = m2rHost

# WAGO register addresses of the QC ADC module and the RTD sensor module
...
cadence = 1.0 # measurement cadence in seconds
numMeas = 50 # number of measures (-1 for infinite loop)
...
logStr = f"{qc1:.5f}  {qc2:.5f}  {qc3:.5f}  {qc4:.5f}"
...
while(True):
   ...
   print(f"{logStr}")
   measCount += 1
   if numMeas > 0 and measCount >= numMeas:
       break

if numMeas > 0:
   medQ1 = np.median(qc1d)
   sigQ1 = np.std(qc1d)
   medQ2 = np.median(qc2d)
   sigQ2 = np.std(qc2d)
   medQ3 = np.median(qc3d)
   sigQ3 = np.std(qc3d)
   medQ4 = np.median(qc4d)
   sigQ4 = np.std(qc4d)
   print(f"QC1: {medQ1:.4f} +/- {sigQ1:.4f}")
   print(f"QC2: {medQ2:.4f} +/- {sigQ2:.4f}")
   print(f"QC3: {medQ3:.4f} +/- {sigQ3:.4f}")
   print(f"QC4: {medQ4:.4f} +/- {sigQ4:.4f}")
