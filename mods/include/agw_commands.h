...
int cmd_agwfilt (char *, MsgType, char *); // set/query acquisition/guide camera filter
int cmd_agwxy   (char *, MsgType, char *); // set/query absolute X,Y stage position
int cmd_agwdxy  (char *, MsgType, char *); // offset the X,Y stage relative to current position
int cmd_agwfocus(char *, MsgType, char *); // set/query absolute camera focus
int cmd_agwdfoc (char *, MsgType, char *); // step the camera focus in/out relative to the current focus
int cmd_autofoc (char *, MsgType, char *); // set/query stage autofocus
int cmd_agwhome (char *, MsgType, char *); // home the AGW stage
int cmd_agwcen  (char *, MsgType, char *); // center the AGW stage in the science field
...
int cmd_agwreset(char *, MsgType, char *); // reset the AGW (warm restart)
...
cmdtab[] = {   //!< global scope command table for this application
  {"quit"    ,cmd_quit    ,"quit","Terminate the AGW command server session"},
  {"reset"   ,cmd_agwreset,"reset","Reload the runtime config file and reset the AGW stage"},
  {"abort"   ,cmd_abort   ,"abort","Abort all AGW stage motions in progress and idle"},
  ...
  {"status"  ,cmd_status  ,"status","Query current ISL AGW stage status"},
  {"agwfilter",cmd_agwfilt,"agwfilter <n>","Set/Query the A/G camera filter (range: 1..4)"},
  {"agwxy"   ,cmd_agwxy   ,"agwxy <x y>","Set/Query the AGW stage absolute X,Y position in millimeters"},
  {"agwdxy"  ,cmd_agwdxy  ,"agwdxy <dx dy>","Offset the AGW stage by dx,dy millimeters from the current position"},
  {"agwhome" ,cmd_agwhome ,"agwhome","Home the AGW stage to its stowed, safe position."},
  {"agwcenter",cmd_agwcen ,"agwcenter","Center the AGW stage probe in the science field."},
  {"agwfocus",cmd_agwfocus,"agwfocus <f>","Set/Query the AGW camera focus in absolute focus units (mm)"},
  {"agwdfoc" ,cmd_agwdfoc ,"agwdfoc <df>","Step the AGW camera focus in/out by df millimeters"},
  {"autofocus",cmd_autofoc,"autofocus <on|off|zero>","Set/Query the autofocus state (on/off)"},
  ...
  {"driftvec",cmd_driftvec,"driftvec <xdot ydot>","Set/Query the AGW stage drift vector in mm/sec"},
