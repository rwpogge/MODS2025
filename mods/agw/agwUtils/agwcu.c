/** \file agwcu.c
 * \brief All purpose client for sending an ASCII string to
 * agwServer. agwServer sends information.
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <memory.h>

#include "../include/islapi.h"
#include "../include/modscontrol.h"

#define BUFFSIZE		512
extern char *mods_fatalerrormsg[];

int agwcu(char *host, int agw_no, char who[], char str1[])
{
  islcomp comp;
  islnum app;
  islconn conn;
  char errchar[4];
  char send_buff[BUFFSIZE];
  char recv_buff[BUFFSIZE];
  int i;
  int len;
  int bcnt;
  int ierr;

  memset(send_buff,0,sizeof(send_buff));
  memset(recv_buff,0,sizeof(recv_buff));
 
  ierr=0;
   comp = islCnameToComp(host); // convert arguments to binary format comp
  if (comp == -1) {
    ierr=1;
    sprintf(recv_buff,"Network Error, *NO* such host named <%s>",host);
    mods_fatalerrormsg[ierr]=(char *)recv_buff;
    return(ierr);
  }

  if (MODS_1_PORT != 0)
    if(agw_no==0)
      app = (islnum) MODS_1_PORT; // convert argument to binary format islnum
    else
      app = (islnum) MODS_2_PORT; // convert argument to binary format islnum
  else
    if ((app = islAppNameToAppNumb("echo")) == -1)
      return(2);
  
  //app = (islnum) atoi(str); // convert argument to binary format islnum

  if (app == -1) {
    ierr=2;
    sprintf(recv_buff,"Network Error, *NO* such PORT <%d>",app);
    mods_fatalerrormsg[ierr]=(char *)recv_buff;
    return ierr;
  }

  conn = islMakeContact(comp, app);
  if (conn < 0) {
    ierr=3;
    sprintf(recv_buff,"Comm. Error, *NO* Connection formed on <%s>",host);
    mods_fatalerrormsg[ierr]=(char *)recv_buff;
    return ierr;
  }

  (void) fflush(stdout);

  sprintf(send_buff, "%s %s", who, str1);
  len=strlen(send_buff)+1;
  send_buff[len]='\0';

  (void) send(conn, send_buff, len, 0); // send msg to agwService
  (void) fflush(stdout);
      
  len = recv(conn, recv_buff, sizeof(recv_buff), 0); // read from agwService
  recv_buff[len]='\0'; // prevent seg. faults
  
  if (len < 0) {

    sprintf(recv_buff,"Comm. Error: read from <%s> *FAILED*",host);
    mods_fatalerrormsg[ierr]=(char *)recv_buff;
    ierr=4;

  } else if(strstr(recv_buff,"ERROR:")) { // check for error
    for(bcnt=0;recv_buff[bcnt]!=':';bcnt++); // Search for ':' in error string
    bcnt++;
    
    strncpy(errchar,&recv_buff[bcnt],3); // get error number.
    ierr=atoi(errchar);

    mods_fatalerrormsg[1]=(char *)&recv_buff[bcnt+3];
    strcpy(str1,&recv_buff[bcnt+3]); // return the string to caller

    //    ierr = -1;

  } else {

    strcpy(str1,recv_buff); // return the string to caller
    ierr = 0;

  }

  (void) fflush(stdout);
  (void) islSendEOF(conn);  // iteration ends when EOF found on stdin

  return ierr;
}
