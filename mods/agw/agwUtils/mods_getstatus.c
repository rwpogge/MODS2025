/** \file mods_getstatus.c
 * \brief Client for obtaining the status of the guiding mirror of the
 * specified MODS AGW Unit via the agwServer.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int 
mods_getstatus(char *host, int agw_no, struct data *agwdata, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (len < 0) {
    ...
    sprintf(agwdata->general_state,"%s",recv_buff);
    agwdata->temp_mods=-1;
    agwdata->temp_outside=-1;
    agwdata->hum_mods=-1;
    ierr=0;
  }  
