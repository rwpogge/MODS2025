/** \file CCDtoSFP.c
 * \brief Client for get GP X and Y values from CCD
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int
CCDtoSFP(char *host, int agw_no, double xgp, double ygp, double dxref, double dyref, struct tc_xy *pxy, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  sprintf(send_buff,"ccdtosfp %d %f %f",agw_no, xgp,ygp);
  ...
  if (len < 0) {
    ...
    pxy->dsfp_x=atof(holder);
    GetArgs(recv_buff,5,holder);
    pxy->dsfp_y=atof(holder);
