...
int
mods_AGW2SFP(char *host, int agw_no, double xgp, double ygp, struct tc_xy *pxy, int options)
{
  islcomp comp;
  islnum app;
  islconn conn;
  ...
  if (len < 0) {
    ...
    GetArg(recv_buff,3,holder);
    pxy->sfp_x=atof(holder);
    GetArg(recv_buff,4,holder);
