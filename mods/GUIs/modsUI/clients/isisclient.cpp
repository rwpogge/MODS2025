/*!
  ...
  There are three ways to send commands:
    1) Immediate - send it directly to the socket and don't wait
    2) Send-and-Wait - send immediately and wait for a reply (block)
    3) Queue - queue the message to the dispatcher thread (non-blocking)
  The dispatcher thread is a simple message dispatcher queue that will
  dispatch each message at a given cadence.
  ...
  : QWidget(parent)
{
  ...
  // Setup but do not yet enable the heartbeat timer and its defaults

  heartBeatTimer = new QTimer(this);
  connect(heartBeatTimer,SIGNAL(timeout()),this,SLOT(sendHeartBeat()));
  heartBeat = false;    // disable the heartbeat by default
  heartBeatTime = 120;  // default heartbeat time is 120 seconds
...
// and sets up the dispatcher queue and the heartbeat timer.
//
...
bool ISISClient::init()
{
  ...
  // Connect the command sequencer thread's dispatch() signal to
  // our sendDispatch() slot, and set the cadence to the default value
  // of 1 second (1000 msec)

  msgQueue.setCadence(1000);
  connect(&msgQueue,SIGNAL(dispatch(const QString &, const QString &)),
	  this,SLOT(sendDispatch(const QString &, const QString &)));

  // If heartbeat is enabled, start the heartbeat timer

  if (heartBeat) enableHeartBeat(true);
...
{
  // Do not send unaddressed messages...
...
//
//    fromID>destID msgType: body of the message\r
//
// where:
//   fromID  = ID of the sender
//   destID  = ID of the recipient
//   msgType = type of message, one of REQ:, EXEC:, DONE:, STATUS:, WARNING:, 
//                  ERROR:, or FATAL:
//   followed by the body of the message, the first word of which is the
//   command string, except for ping/pong and broadcast messages.
//
...
// Queue a message to dispatcher thread

void ISISClient::queue(const QString &host, const QString &body)
{
  if (host.isEmpty()) return;
  if (body.isEmpty()) return;
  msgQueue.addMessage(host,body);
}

// Send a message from the dispatcher queue (implemented as a private slot)

void ISISClient::sendDispatch(const QString &host, const QString &body)
{
  if (!send(host,body)) emit error();
}

// Set the message dispatcher queue cadence in milliseconds

void ISISClient::setDispatchCadence(const int &cadence)
{
  if (cadence > 0)
    msgQueue.setCadence(cadence);
}

// Abort the message dispatcher queue - implemented as a public slot

void ISISClient::abortQueue()
{
  msgQueue.abort();
}
...
void ISISClient::enableHeartBeat(const bool &hbState)
{
  if (hbState) {
    heartBeatTimer->stop();  // just in case
    if (heartBeatTime > 0) {
      heartBeat = true;
      heartBeatTimer->start(heartBeatTime*1000);
    }
  }
  else {
    heartBeatTimer->stop();
...
// Set the heartBeat time interval in seconds.  If set to zero, it also 
// disables the heartbeat timer

void ISISClient::setHeartBeatTime(const int &hbTime)
{
  if (hbTime <= 0) {  // zero or negative means disable the heartbeat timer
    heartBeatTimer->stop();
    heartBeatTime = 0;
    heartBeat = false; 
  }
    ...
    if (heartBeat) {  // restart the heartbeat timer if running
      heartBeatTimer->stop();
      heartBeatTimer->start(hbTime*1000);
    }
  }
}

// Send the heartbeat message to the ISIS server.   This is a private
// slot connected to the timeout() signal of the heartBeatTimer()
// Heartbeats are prompt dispatch messages.

void ISISClient::sendHeartBeat()
{
  if (!isisServerID.isEmpty())
    send(isisServerID,"");
}
