...
global mAval mBval mCval xnew ynew ynew2 ival fval t1val tmval whichmask
...
# bind $w.d1ent <Return> { set xdsp $d1val;}
 ...
 set q1val [expr [exec vueinfo mlc21]] 
 set q2val [expr [exec vueinfo mlc20]]
 set q3val [expr [exec vueinfo masksma]]
 set q4val [expr [exec vueinfo mlc22]]
...
name)]
 ...
 #
    ...
    set xnew [expr [ exec vueinfo mlc21 ]];
    set ynew [expr [ exec vueinfo mlc20 ]];
    set znew [expr [ exec vueinfo mlc22 ]];
...
#     worldCoordinates $w 0.0 10.0 0.0 10.0
 ...
 entry .m4Went -textvariable ttfW -font $bfont -relief flat -bg white -width 10

 pack  .m4Xlab .m4Xent -side left
 pack  .m4Ylab .m4Yent -side left
 pack  .m4Flab .m4Fent -side left
 pack  .m4Wlab .m4Went -side left
 bind  .m4Yent <Return> { exec vueinfo mmcu agwy $ttfY };
 bind  .m4Xent <Return> { exec vueinfo mmcu agwx $ttfX  };
 bind  .m4Fent <Return> { exec vueinfo mmcu agwfoc $ttfF };
 bind  .m4Went <Return> { exec vueinfo mmcu agwfilt $ttfW };

proc islanadata { } {
    global xdelx ydely colourq xred yred xreda yreda pfnm gblst ival
    global mAval mBval mCval dimmer whichmask
...
#    set zred [expr abs(([expr [exec vueinfo agwval3]])/[expr 36.00])]
    set xred [expr abs(([expr [exec vueinfo mlc21]])/[expr 36.00])]
    set yred [expr abs(([expr [exec vueinfo mlc20]])/[expr 36.00])]
    set zred [expr abs(([expr [exec vueinfo mlc22]])/[expr 36.00])]
...
#    set whichmask [exec vueinfo masksmA]
    set whichmask [exec vueinfo mlc28]
