...
# bind $w.t1ent <Return> { exec vueinfo imcst1 $t1val; }
 ...
 proc changeColor { w chgc } { 
     if { $chgc ==  1 } {
         $w configure -text "Close Loop" -background red;
     } else {
         $w configure -text "Open Loop" -background green;
     }
 }
...
name)]
 ...
 label $w.m4lab2 -text "bcolfoc reset\nbcolfoc ttfa ttfb ttfc\nbcolfoc step +/-ttfa +/-ttfb +/-ttfc" -font $bfont3  -bg white -justify left
 ...
 bind  $w.m4ent <Return> {  \
	...
	if { $arg1 == "step" } {
	  ...
	  $w.m4lab2 configure -bg grey;
	    exec islmlynxShm "$arg $arg1 $arg2 $arg3 $arg4";
	} elseif { $arg1 == "reset" } {
	  set w .5.2.gp5
	  $w.m4lab2 configure -bg grey;
	    exec islmlynxShm "$arg $arg1";
	} else {
	  if { [expr $arg1 < 0] || [expr $arg2 < 0] || [expr $arg3 < 0]} {
	   ...
	   set w .5.2.gp5
	   $w.m4lab2 configure -bg grey;
	    exec islmlynxShm "$arg $arg1 $arg2 $arg3";
	  };
	};
 };
