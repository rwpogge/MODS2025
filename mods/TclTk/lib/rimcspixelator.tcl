...
# bind $w.t1ent <Return> { exec vueinfo imcst1 $t1val; }
 ...
 set q1val [expr [exec vueinfo qcell5]] 
 set q2val [expr [exec vueinfo qcell6]]
 set q3val [expr [exec vueinfo qcell7]]
 set q4val [expr [exec vueinfo qcell8]]
 set q5val [expr [exec vueinfo average_qcell5]]
 set q6val [expr [exec vueinfo average_qcell6]]
 set q7val [expr [exec vueinfo average_qcell7]]
 set q8val [expr [exec vueinfo average_qcell8]]
 set mAval [expr [exec vueinfo osci4]] 
 set mBval [expr [exec vueinfo osci5]]
 set mCval [expr [exec vueinfo osci6]]
 set ttfA [expr [exec vueinfo imcsacc4]] 
 set ttfB [expr [exec vueinfo imcsacc5]]
 set ttfC [expr [exec vueinfo imcsacc6]]
 set xdsp $d1val
 set w .5.2
 frame $w.gp2 -borderwidth 4; pack $w.gp2 -side top
 set w .5.2.gp2

 label $w.q1lab -font $bfont -text "QC 1:" -fg red
 label $w.q1val -textvariable q1val -font $bfont -background black -fg white -width 8
 label $w.q2lab -font $bfont -text "QC 2:" -fg red
 label $w.q2val -textvariable q2val -font $bfont -background black -fg white -width 8
 label $w.q3lab -font $bfont -text "QC 3:" -fg red
 label $w.q3val -textvariable q3val -font $bfont -background black -fg white -width 8
 label $w.q4lab -font $bfont -text "QC 4:" -fg red
 ...
 button $w.cloop -text "Open Loop" -relief {raise} \
    -background linen -fg black -command { \
       global bgc;
	if { $bgc == 1 } { 
	    exec vueinfo closeLoopOFF1; 
	    set bgc 0; 
	    set w .5.2.gp4
	    $w.cloop configure -text "Close Loop" -background red;
	} else {
	    exec vueinfo closeLoopON1; 
	    set bgc 1; 
	    set w .5.2.gp4
...
# pack $w.mClab $w.mCval -side left
...
name)]
 ...
 #
    ...
    set ttfT [exec vueinfo sthresholdM];

    set xnew [expr [ exec vueinfo osci1 ]];
    set ynew [expr [ exec vueinfo osci2 ]];
 ...
 moveto  [expr [exec vueinfo osci1]] [expr [exec vueinfo osci2]]
 }
 ...
 label $w.m4lab2 -text "rcolfoc reset\nrcolfoc ttfa ttfb ttfc\nrcolfoc step +/-ttfa +/-ttfb +/-ttfc" -font $bfont3  -bg white -justify left
 ...
 bind  $w.m4ent <Return> {  \
	...
	if { $arg1 == "step" } {
	  ...
	  $w.m4lab2 configure -bg grey;
          exec imcsCtl $arg $arg1 $arg2 $arg3 $arg4; 
	} elseif { $arg1 == "reset" } {
	  set w .5.2.gp5
	  $w.m4lab2 configure -bg grey;
	  exec imcsCtl $arg $arg1 $arg2 $arg3 $arg4; 
	} else {
	  if { [expr $arg1 < 0] || [expr $arg2 < 0] || [expr $arg3 < 0]} {
	   set w .5.2.gp5
...
#    puts "$xgray $ygray $xblue $yblue [expr [exec vueinfo osci1]] [expr [exec vueinfo osci2]]"    
...
#    draw grid
    ...
    set testC "11"
    set testN "01"
    set loopStatus [expr [exec vueinfo rcloseLoop1] ];
    if { $loopStatus == $testO } {
	set w .5.2.gp4
	$w.cloop configure -text "Open Loop" -background green;
	...
	set w .5.2.gp4
	$w.cloop configure -text "Recovering Close Loop" -background red;
    }
