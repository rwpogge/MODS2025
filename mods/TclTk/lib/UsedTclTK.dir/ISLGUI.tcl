...
# all of the isl gui's as hypertext items.
#----------------------------------------------------------------
...
# section titles and GUI descriptions.  Also define the bindings for
# tags.

.t tag configure title -font $widgetFont(title)
.t tag configure bold  -font $widgetFont(bold)

# I've put some "space" characters to the left and right of each GUI
# description so that the descriptions are highlighted only when the mouse
# cursor is right over them (but not when the cursor is to their left or right)
#
...
proc addGuiSection {title demos} {
    .t insert end "\n" {} $title title " \n " demospace
    set num 0
...
.t insert end "Tk OSU/ISL GUI's\n" title
.t insert end "\nThis application provides a front end for several short\
	scripts that demonstrate what you can do with ISL widgets.  Each of\
	...
	edit the code and click the " {} "Rerun" bold " button in the\
	code window to reinvoke the demonstration with the modified code.\n"

addGuiSection "Labels, buttons, checkbuttons, and radiobuttons" {
...
addGuiSection "Entries and Spin-boxes" {
...
addGuiSection "Scales" {
    hscale	"Horizontal scale"
    vscale	"Vertical scale"
}
addGuiSection "Paned Windows" {
...
addGuiSection "Common Dialogs" {
    msgbox	"Message boxes"
    filebox	"File selection dialog"
    clrpick	"Color picker"
}
addGuiSection "Miscellaneous" {
...
proc showStatus index {
    ...
    set cursor [.t cget -cursor]
    if {$i < 0} {
	.statusBar.lab config -text "$filename"
	set newcursor xterm
    } else {
	set demo [string range [lindex $tags $i] 5 end]
	.statusBar.lab config -text " "
...
proc showCode w {
	...
	button .code.buttons.rerun -text "Rerun" -command {
...
#	    -row 1 -column 0 -rowspan 1 -columnspan 1 -sticky news
    ...
    wm title .code "ISL code: [file join $tk_library demos $file]"
...
#
proc aboutBox {} {
    tk_messageBox -icon info -type ok -title "About Widget ISL" -message \
"Tk OSU/ISL widgets
