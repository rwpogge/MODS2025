...
isisbusy(int controller)
{
  ...
  //    shm_addr->MODS.HOST[controller]=0;
  ...
  shm_addr->MODS.busy[controller]=0;
  ierr = 0;
  //  sprintf(monit,"[%s]DONE!",&shm_addr->MODS.WHO[controller]);
  //  logitf(monit);
  return (ierr);
}
